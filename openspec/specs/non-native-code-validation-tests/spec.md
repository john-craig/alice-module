# Spec: non-native-code-validation-tests

## Purpose

Defines the `nix flake check` tests that validate non-native workspace constraints and provisioning behaviour in `flake.nix`.
